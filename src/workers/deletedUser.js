...
const { redisConfig, deletedUserQueue } = require("../config/worker");
...
class DeletedUser {
  ...
  initializeJobProcessing() {
    this.queue.process(async (job) => {
      ...
      try {
        await emailService.sendDeletionEmail(to, clientName);
  ...
  enqueueDeletedUserEmail(data) {
    this.queue.add(data);
    logger.info(
      `Enqueued suspended user email to ${data.to} - ${data.clientName}`
    );
  }
}

module.exports = new DeletedUser(deletedUserQueue);
